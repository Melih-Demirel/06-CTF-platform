    ...
    .then(async () => {
        ...
        try {
            ...
            userNamespace.on('connection', async(socket : Socket) => {
                ...
                const user : User = (await CTFDataSource.getRepository(User).findOne({where: {Id: userId}, select: ["Team"]}));
                if(user?.Team){
                ...
                socket.on('create-team', async(userId) => {
                    const user : User = (await CTFDataSource.getRepository(User).findOne({where: {Id: userId}}));
                ...
                socket.on('new-solve', async() => {
                    const teamId = ((await CTFDataSource.getRepository(User).findOne({where: {Id: userId}})) as User)?.Team.Id.toString();
                    userNamespace.to(teamId.toString()).emit("new-solve");
                    adminNamespace.emit("new-team");
                    userNamespace.emit("new-team");
                })
                ...
                socket.on('disconnect', async() => {
                    await removeSocketIdFromUser(socket.id, userId);
                    const user = await CTFDataSource.getRepository(User).findOne({where: {Id: userId}});
                    
                    if(user?.Team){
                        socket.leave(user.Team.Id.toString());
                    }
                })
            })
