...
export const loadTestData = async () => {
    ...
    // Check if data is already loaded in by checking on admin account
    const admin = await (CTFDataSource.getRepository(User)).findOne({ where: { Email: "Admin@student.uhasselt.be" } });
    if (admin) return;
